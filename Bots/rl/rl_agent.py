...
class StateProcessor:
    ...
    def __init__(self, max_grid_size=50):
        ...
        self.previous_zookeeper_positions = deque(maxlen=5)  # Track previous zookeeper positions
    ...
    def process_state(self, game_state, bot_id):
        ...
        distance_to_zookeepers = self._calculate_distance_transform(~zookeepers.astype(bool))
...
class DQNAgent:
    ...
    """
    def __init__(self, state_shape, action_size=4, learning_rate=0.001, 
                 gamma=0.99, epsilon=1.0, epsilon_decay=0.995, 
                 epsilon_min=0.1, memory_size=10000, batch_size=32,
                 update_target_freq=1000, model_dir='models'):
    ...
    def _build_model(self):
        ...
        x = tf.keras.layers.Conv2D(32, (3, 3), activation='relu', padding='same')(grid_input)
...
class RewardCalculator:
    ...
    def calculate_reward(self, game_state, bot_id, state_processor):
        ...
        if pellet_cells:
            min_distance = min(abs(c.X - our_animal.X) + abs(c.Y - our_animal.Y) 
                              for c in pellet_cells)
        ...
        if len(state_processor.previous_positions) >= 4:
            ...
            if last_positions[0] == last_positions[2] and last_positions[1] == last_positions[3]:
...
class RLBotService:
    ...
    def ProcessState(self, game_state):
        ...
        if self.agent is None:
            ...
            grid_features, metadata = self.state_processor.process_state(game_state, self._bot_id)
        ...
        if self.previous_state is not None and self.previous_action is not None and self.training:
            reward = self.reward_calculator.calculate_reward(
                game_state, self._bot_id, self.state_processor)
            
            # Check if episode ended (animal was captured)
            our_animal = next((a for a in game_state.Animals if a.Id == self._bot_id), None)
            done = False
            if our_animal and our_animal.CapturedCounter > self.reward_calculator.previous_captured:
                done = True
            ...
            if done:
                ...
                print(f"Episode {self.episode_counter} ended with reward: {self.episode_rewards[-1]:.2f}")
                ...
                if current_time - self.last_save_time > 300:  # Save every 5 minutes
                    ...
                    # Save training stats
                    with open(os.path.join(self.agent.model_dir, 'training_stats.json'), 'w') as f:
                        json.dump({
