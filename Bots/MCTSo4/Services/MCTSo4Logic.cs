            ...
            _connection.On<MCTSGameState>(
                ...
                async state =>
                {
                    try
                    {
                        ...
                        var meta = AdaptiveStrategyController.DetermineCurrentMetaStrategy(state);
                        ...
                        else if (stopwatch.ElapsedMilliseconds < parameters.MaxTimePerMoveMs * 0.8)
                        {
                        ...
                        try
                        {
                            _log.Debug("Attempting to send BotCommand: {@BotCommand}", botCommand);
