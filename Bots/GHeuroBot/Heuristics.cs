...
public static class Heuristics
{
    ...
    )
    {
        ...
        score += HeuristicsImpl.PathSafetyAssessment(state, me, nx, ny) * WEIGHTS.PathSafetyScore;
    ...
    internal static class HeuristicsImpl
    {
        ...
        )
        {
            ...
            {
                distanceFactor += 1.0m / (ManhattanDistance(nextX, nextY, pellet.X, pellet.Y) + 1);
            }
        ...
        )
        {
            ...
            foreach (var zk in state.Zookeepers)
            {
                ...
                else if (currentZkTarget?.Id == me.Id && predictedZkTargetAfterMyMove?.Id != me.Id)
                {
        ...
        public static decimal PathSafetyAssessment(GameState state, Animal me, int nextX, int nextY)
        {
        ...
        public static decimal StrategicTerritoryValue(GameState state, int nextX, int nextY)
        {
            ...
            {
                territoryScore += 1.0m / (ManhattanDistance(nextX, nextY, pellet.X, pellet.Y) + 1);
            }
