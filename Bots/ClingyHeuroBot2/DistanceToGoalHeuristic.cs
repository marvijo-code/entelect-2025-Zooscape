    ...
    public class DistanceToGoalHeuristic : IHeuristic
    {
        ...
        public decimal CalculateRawScore(GameState state, Animal me, BotAction move)
        {
            ...
            var closestPellet = pellets
                .OrderBy(c => Heuristics.HeuristicsImpl.ManhattanDistance(c.X, c.Y, nx, ny))
                .FirstOrDefault();
            ...
            var minDistToAnyPelletAfterMove = pellets.Min(c =>
                Heuristics.HeuristicsImpl.ManhattanDistance(c.X, c.Y, nx, ny)
            );
            return -(decimal)minDistToAnyPelletAfterMove * 0.5m;
        }
    }
}
